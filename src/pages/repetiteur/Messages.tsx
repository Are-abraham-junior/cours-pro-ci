...
export default function RepetiteurMessages() {
  const { user } = useAuth();
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      const { data: contracts, error: contractsError } = await supabase
        .from('contracts')
        .select('id, matiere, niveau, parent_id')
      ...
      const parentIds = [
        ...(contracts?.map((c) => c.parent_id) || []),
        ...(directs?.map((d) => d.parent_id) || [])
      ];
      ...
      const contractConvs: Conversation[] = await Promise.all(
        (contracts || []).map(async (contract) => {
          ...
          const profile = profilesMap.get(contract.parent_id);
          const lastMsgData = lastMsgResult.data;
          return {
            chatId: contract.id,
            otherUser: profile || { id: contract.parent_id, full_name: 'Parent', avatar_url: null },
      ...
      const directConvs: Conversation[] = await Promise.all(
        (directs || []).map(async (direct) => {
          ...
          const profile = profilesMap.get(direct.parent_id);
          const lastMsgData = lastMsgResult.data;
          return {
            chatId: direct.id,
            otherUser: profile || { id: direct.parent_id, full_name: 'Parent', avatar_url: null },
      ...
      if (!selectedChatId && allConvs.length > 0) {
        ...
        navigate(first.type === 'contract' ? `/repetiteur/messages/${first.chatId}` : `/repetiteur/messages/direct/${first.chatId}`);
      }
    } finally {
      setLoading(false);
    }
          ...
          <div className="w-full md:w-80 lg:w-96 flex flex-col border-r border-border bg-white dark:bg-zinc-900 shrink-0">
            ...
            <ConversationList
              ...
              onSelect={(id) => {
                ...
                navigate(conv?.type === 'contract' ? `/repetiteur/messages/${id}` : `/repetiteur/messages/direct/${id}`);
              }}
              loading={loading}
            />
          </div>
